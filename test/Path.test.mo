      ...
      let result = switch (Path.fromText(testCase.input)) {
        ...
        case (#err(errMsg)) {
          if (testCase.expected != null) {
            Runtime.trap(formatError("Path.fromText error", testCase.input, "Expected success", debug_show (errMsg)));
          };
          continue f; // Valid
        };
      };
      ...
      switch (testCase.expected) {
        case (?expectedPath) {
          if (result != expectedPath) {
            Runtime.trap(formatErrorArray("Path.fromText", testCase.input, expectedPath, result));
          };
        };
        case (null) {
          ...
          Runtime.trap("Test case error: null expected value not supported for Path.fromText");
        };
      };
    };
  },
  ...
  func() {
    ...
    for (testCase in testCases.vals()) {
      ...
      if (result != testCase.expected) {
        Runtime.trap(formatError("Path.toText", debug_show (testCase.input), testCase.expected, result));
      };
    };
  },
);
